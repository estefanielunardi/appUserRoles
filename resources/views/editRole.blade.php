<x-app-layout>
    ...
    <section class="flex flex-col m-10">
        <h1 class="title pb-8 text-4xl">Edit Role</h1>
    </section>
        ...
        <div class="box-border p-4 bg-white h-128 w-96">
            <form method="POST" action="{{route('updateRole' , $role->id)}}">
                ...
                <div class="flex flex-col my-4 text-xl">
                    ...
                    <input type="text" id="title" class="w-100 border-solid border-2 rounded shadow-md h-10" name="title" value="{{$role->title}}" required autocomplete="title" autofocus>
                </div>
                <div class="flex justify-center">
                    <button type="submit" class="bg-blue-400 antialiased text-black text-2xl mt-4 px-12 py-4 rounded-xl shadow-md">{{ __('Edit Role') }}</button>
                </div>
            </form>
        </div>
    </section>
